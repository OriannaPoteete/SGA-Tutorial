...
Here we use $\Delta$ as an example and provide "del" as a suggestion for replacing it.
...
* raw_data_colnames <- this prints the changed dataframe to see if we replaced $\Delta$ in our column names


## Averaging Blank Measurements for Removing Background Noise

Now that we have formatted our dataframe, we can begin the analysis.First, we need to subtract the background noise from our data.

For each timepoint, we took a measurement of 
